...
pub enum BaseError {        // TODO описать Контектс там, где он нцжен для понимания информации.
    ...
    InsufficientAttachedNearForDistributionOnValidators,
    ...
    NearAmountIsTooBig,
    InvestmentWithdrawalIsNotRegistered,
    SomeFundIsNotUnstaked
}
...
impl Display for BaseError {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> Result<(), FmtError> {
        match *self {
            ...
            Self::RemovingInvestorWithDistribution => {
                formatter.write_str("todo.")?;
            }
            Self::InsufficientAttachedNearForDistributionOnValidators => {
                formatter.write_str("todo.")?;
            }
            ...
            Self::NearAmountIsTooBig => {
                formatter.write_str("todo.")?;
            }
            Self::InvestmentWithdrawalIsNotRegistered => {
                formatter.write_str("todo.")?;
            }
