...
impl ValidatingNode {
    ...
    pub fn register_validator_account(&mut self, account_id: &AccountId) -> Result<(), BaseError> {
        ...
        if let Some(_) = self.validator_account_registry.insert(account_id, &ValidatorInfo::new()) {
...
// Поставить проверки, чтоюы никуда не зашел ноль !!!!!!!!!!!!!!!!!!!!!!!
    ...
    fn calculate_storage_usage_per_additional_validator_account() -> Result<StorageUsage, BaseError> {
        ...
        validator_account_registry.insert(&account_id, &ValidatorInfo::new());
